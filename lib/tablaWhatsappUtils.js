...
// Formato corto sin año ni coma, ej. "miércoles 06 de agosto" (usado en la
// página pública de registro de entregas, donde el año es implícito).
export function formatFechaDiaMes(fechaISO) {
  const [y, m, d] = fechaISO.split('-').map(Number)
  const dt = new Date(y, m - 1, d)
  const diaSemana = DIAS_SEMANA[dt.getDay()]
  const mes = MESES[m - 1]
  return `${diaSemana} ${String(d).padStart(2, '0')} de ${mes}`
}

// Formato largo sin coma, ej. "miércoles 06 de agosto de 2026" (usado en el
// mensaje de cierre de ruta enviado por WhatsApp).
export function formatFechaLargaSinComa(fechaISO) {
  const [y, m, d] = fechaISO.split('-').map(Number)
  const dt = new Date(y, m - 1, d)
  const diaSemana = DIAS_SEMANA[dt.getDay()]
  const mes = MESES[m - 1]
