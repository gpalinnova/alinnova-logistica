...
function drawHeaderBlock(doc, y, { familia, fechaGeneracion }) {
  const logoW = 40
  const rightW = 60
...
function drawTablaHeader(doc, y, productos, layout) {
  ...
  let ex = embalajeX0
  for (const p of productos) {
    drawCell(doc, ex, y + h1 + h2, embalajeW, h3, `x${p.capacidad_canastilla || '-'}`, { bold: true, fontSize })
    drawCell(doc, ex, y + h1 + h2 + h3, canastillaW, h4, 'CANASTILLA', { bold: true, fontSize: 4.4 })
    drawCell(doc, ex + canastillaW, y + h1 + h2 + h3, undW, h4, 'UND', { bold: true, fontSize: 4.4 })
    ex += embalajeW
  }
...
function drawPagina(doc, { familia, ruta, fechaEntrega, fechaGeneracion, sitios, productos_variantes, lotes_venc }) {
  ...
  let y = MARGIN
  y = drawHeaderBlock(doc, y, { familia, fechaGeneracion })
...
export function generarPdfRuteros(ruteros, modo) {
  ...
  return ruteros.map(r => {
    const doc = new jsPDF({ unit: 'mm', format: 'a4', compress: true })
    construirHtmlRutero(doc, r)
    return { ruta: r.ruta, familia: r.familia, blob: doc.output('blob') }
  })
}
