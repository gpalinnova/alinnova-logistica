...
export async function getRepartidoresActivosFecha(fecha) {
  ...
  const [{ data: asignaciones }, { data: baseRows }, { data: sitiosData }] = await Promise.all([
    supabase
      ...
      .select('id_sitio_entrega, orden_entrega, repartidor:reforzados_repartidores(id, conductor, auxiliar, placa)')
  ...
  for (const r of repartidores) r.sitios.sort((x, y) => x.orden - y.orden)
