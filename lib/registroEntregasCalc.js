...
// colegio (nombre) y horario esperado — regla automática por clave
// nombre+horario, sin tabla ni lista manual: si el patrón de colegios se
// repite en próximos meses, la unificación aplica sola. Se usa solo para
// armar las tarjetas del formulario del conductor (getDatosDelDia en
...
export function agruparSitiosParaTarjetas(sitios) {
  const grupos = new Map()
  for (const s of sitios) {
    const clave = `${(s.nombreInstitucion || '').trim().replace(/\s+/g, ' ').toUpperCase()}|${s.horarioEsperadoMinutos ?? ''}`
