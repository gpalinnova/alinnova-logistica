...
const SUMINISTRO_ADICIONAL_RE = /SE\s+ENVIAN\s+(\d+)\s+SUMINISTROS?\s+ADICIONALES?\s*\(\s*TIPO\s*([12])\s*\)/g

// Detecta líneas tipo "SE ENVIAN N SUMINISTRO(S) ADICIONAL(ES) (TIPO X)" en la
// observación de Base de Suministro (tolerante a acentos, mayúsculas y
// singular/plural). Devuelve un array de matches en el orden en que aparecen.
export function extractSuministrosAdicionales(observacion) {
  const norm = stripAccents(observacion)
  const matches = []
  const re = new RegExp(SUMINISTRO_ADICIONAL_RE.source, 'g')
  let m
  while ((m = re.exec(norm)) !== null) {
    matches.push({ cantidad: parseInt(m[1], 10), tipo: m[2] })
  }
  return matches
}
