...
//
// Tipo 1 = tipo_a + tipo_b + muestra_tipo_1 (columna generada tipo_1_total).
// Tipo 2 = tipo_c + tipo_d + muestra_tipo_2 (columna generada tipo_2_total).
// tipo_n (formato B de Base de Suministro) no tiene columna dedicada en el
// reporte, así que se asume que corresponde al componente grande y se suma a
// Tipo 2.
export function buildRutasData(asignaciones, sitiosById, baseByIdSitio) {
  ...
  for (const a of asignaciones) {
    ...
    if (!g.cargueAlinnova && a.cargue_alinnova) {
      g.cargueAlinnova = a.cargue_alinnova
    ...
    g.filas.push({
      ...
      horarioSugerido: sitio?.horario_sugerido || '-',
      horarioEntregaAlinnova: a.horario_entrega_alinnova || '-',
      orden: a.orden_entrega,
      tipo1: sinBase ? null : base.tipo_1_total,
      tipo2: sinBase ? null : base.tipo_2_total + (base.tipo_n || 0),
      sinBase,
    })
  }
  ...
  const conductores = Array.from(grupos.values()).map(g => {
    ...
    const subtotalTipo1 = filas.reduce((s, f) => s + (f.tipo1 || 0), 0)
    const subtotalTipo2 = filas.reduce((s, f) => s + (f.tipo2 || 0), 0)
    return { ...g, filas, subtotalTipo1, subtotalTipo2 }
  })
  ...
  const totalTipo1 = conductores.reduce((s, c) => s + c.subtotalTipo1, 0)
  const totalTipo2 = conductores.reduce((s, c) => s + c.subtotalTipo2, 0)

  return { conductores, totalTipo1, totalTipo2 }
}

export function buildRutasAsciiText(tituloTabla, conductores, totalTipo1, totalTipo2) {
  const headers = ['Conductor', 'Auxiliar', 'Placa', 'Cargue Alinnova', 'Id Sitio', 'Nombre y/o Descripción Sitio', 'Dirección', 'Horario Sugerido Compensar', 'Horario Entrega Alinnova', 'Orden', 'Total Tipo 1', 'Total Tipo 2']
  const rows = []
  ...
  for (const c of conductores) {
    for (const f of c.filas) {
      rows.push([
        ...
        f.direccion,
        f.horarioSugerido,
        f.horarioEntregaAlinnova,
        f.sinBase ? `${f.orden} ⚠️` : String(f.orden),
        f.sinBase ? '-' : String(f.tipo1),
        f.sinBase ? '-' : String(f.tipo2),
      ])
    }
    rows.push([`SUBTOTAL ${c.conductor}`, '', '', '', '', '', '', '', '', '', String(c.subtotalTipo1), String(c.subtotalTipo2)])
  }
  rows.push(['TOTAL GENERAL', '', '', '', '', '', '', '', '', '', String(totalTipo1), String(totalTipo2)])

  const lines = []
  lines.push(tituloTabla)
  lines.push('')
...
export function isInterventoria(observacion) {
  return stripAccents(observacion).includes('INTERVENTORIA')
}

export function extractInterventoriaTipo(observacion) {
  const m = stripAccents(observacion).match(/TIPO\D{0,3}([12])\b/)
  return m ? m[1] : null
}
