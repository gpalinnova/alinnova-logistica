...
// de cada conductor, por Id Sitio Entrega ascendente (orden fijo, independiente
// del valor de Orden Entrega del día).
//
// ordenOverridesByIdSitio: Map<idSitioEntrega, ordenEntrega> con los valores
// guardados para la fecha en reforzados_override_orden_entrega. Cada día
// arranca en 0 para todos los sitios hasta que el usuario lo edite.
export function buildRutasData(asignaciones, sitiosById, baseByIdSitio, ordenOverridesByIdSitio) {
  ...
  for (const a of asignaciones) {
    ...
    g.filas.push({
      ...
      institucion: sitio?.nombre_institucion || `ID ${a.id_sitio_entrega} no encontrado`,
      ...
      orden: ordenOverridesByIdSitio?.get(a.id_sitio_entrega) ?? 0,
      sinBase,
    })
  }

  const conductores = Array.from(grupos.values()).map(g => {
    const filas = [...g.filas].sort((a, b) => a.idSitio - b.idSitio)
    return { ...g, filas }
  })
...
const SUMINISTRO_ADICIONAL_START_RE = /SE\s+ENVIA[N]?\s+\d+\s+SUMINISTROS?\s+ADICIONAL(?:ES)?/
const SUMINISTRO_CANTIDAD_RE = /SE\s+ENVIA[N]?\s+(\d+)\s+SUMINISTRO/i

// Detecta líneas tipo "SE ENVIAN N SUMINISTRO(S) ADICIONAL(ES)..." en la
// observación de Base de Suministro (tolerante a acentos, mayúsculas y
// singular/plural). cantidad queda en null si la fila calza el patrón pero
// no se pudo parsear el número (se muestra "?" en pantalla sin romper).
export function extractSuministrosAdicionales(observacion) {
  const norm = stripAccents(observacion)
  if (!SUMINISTRO_ADICIONAL_START_RE.test(norm)) return []
  const m = norm.match(SUMINISTRO_CANTIDAD_RE)
  return [{ cantidad: m ? parseInt(m[1], 10) : null }]
}
...
export function buildInterventorias(baseRows, sitiosById, asignacionBySitio) {
  ...
  for (const row of baseRows) {
    ...
    bloques.push({
      ...
      sinAsignar,
      suministrosAdicionales: extractSuministrosAdicionales(row.observacion),
    })
  }
  return bloques
}
