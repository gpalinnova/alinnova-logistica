...
export function detectarFamilia(nombreProducto) {
  if (!nombreProducto) return 'SIN CLASIFICAR'
  const nombre = String(nombreProducto).toUpperCase().normalize('NFD').replace(/[̀-ͯ]/g, '')
  // Reglas específicas conocidas (más largas primero para no matchear parcial)
  const reglas = [
    { patron: 'SANDUCHE DE CARNE DE CERDO', familia: 'SANDUCHE DE CARNE DE CERDO' },
    { patron: 'HOJALDRE DE BOCADILLO Y QUESO', familia: 'HOJALDRE DE BOCADILLO Y QUESO' },
    { patron: 'WRAP DE POLLO Y QUESO', familia: 'WRAP DE POLLO Y QUESO' },
    { patron: 'MUFFIN SABOR QUESO', familia: 'MUFFIN SABOR QUESO' },
    { patron: 'MUFFIN CON SABOR A QUESO', familia: 'MUFFIN SABOR QUESO' },
    { patron: 'MUFFIN DE QUESO', familia: 'MUFFIN DE QUESO' },
    { patron: 'MUFFIN DE BANANO', familia: 'MUFFIN DE BANANO' },
    { patron: 'PONQUE VAINILLA', familia: 'PONQUE VAINILLA' },
    { patron: 'CROISSANT', familia: 'CROISSANT' },
    { patron: 'PALITO DE QUESO', familia: 'PALITO DE QUESO' },
    { patron: 'PAN DE MAIZ', familia: 'PAN DE MAIZ' },
    { patron: 'PAN CALENTANO', familia: 'PAN CALENTANO' },
    { patron: 'PAN DE BONO', familia: 'PAN DE BONO' },
    { patron: 'PATACON', familia: 'PATACON' },
    { patron: 'FRIJOL', familia: 'FRIJOL' },
    { patron: 'GARBANZO', familia: 'GARBANZO' },
  ]
  for (const r of reglas) {
    if (nombre.includes(r.patron)) return r.familia
  }
  // Fallback: primer segmento antes del primer número o guion
  const match = nombre.match(/^([A-Z\s]+?)(\s+\d|\s*-)/)
...
export function normalizarNombreParaMatch(nombre) {
  return String(nombre || '')
    .normalize('NFD').replace(/[̀-ͯ]/g, '')
    .toUpperCase()
    .replace(/\(IED\)/g, '')
    .replace(/\bCOLEGIO\b/g, 'COL')
    .replace(/[.,\-']/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
}
...
export function construirPreview(lineasValidas, sitiosExistentes, productosExistentes) {
  ...
  const sitiosPorNombreMatch = new Map()
  for (const s of sitiosExistentes) {
    const key = normalizarNombreParaMatch(s.nombre_institucion)
    if (!sitiosPorNombreMatch.has(key)) sitiosPorNombreMatch.set(key, [])
    sitiosPorNombreMatch.get(key).push(s)
  }

  const sitiosNuevosMap = new Map()
  const sitiosConflictoNombreMap = new Map()
  const sitiosActualizarWmsMap = new Map()
  ...
  for (const linea of lineasValidas) {
    ...
    if (!sitioExistente) {
      const candidatos = sitiosPorNombreMatch.get(normalizarNombreParaMatch(linea.nombre_bodega)) || []
      const lineaLocalidad = normalizarTexto(linea.localidad)
      const sitioPorNombre = candidatos.find(c => {
        const sitioLocalidad = normalizarTexto(c.localidad)
        return sitioLocalidad === lineaLocalidad || sitioLocalidad === 'SIN LOCALIDAD' || lineaLocalidad === 'SIN LOCALIDAD'
      })
      if (sitioPorNombre) {
        if (!sitiosActualizarWmsMap.has(linea.punto_wms)) {
          sitiosActualizarWmsMap.set(linea.punto_wms, {
            sitio_existente_id: sitioPorNombre.id,
            sitio_existente_wms: sitioPorNombre.punto_wms,
            sitio_existente_nombre: sitioPorNombre.nombre_institucion,
            wms_nuevo: linea.punto_wms,
            nombre_nuevo: linea.nombre_bodega,
            localidad: linea.localidad,
          })
        }
      } else if (!sitiosNuevosMap.has(linea.punto_wms)) {
      ...
      if (!sitiosConflictoNombreMap.has(linea.punto_wms)) {
        sitiosConflictoNombreMap.set(linea.punto_wms, {
          punto_wms: linea.punto_wms,
    ...
    if (!productoExistente && !productosNuevosMap.has(linea.codigo_articulo)) {
      productosNuevosMap.set(linea.codigo_articulo, {
        ...
        familia_detectada: detectarFamilia(linea.nombre_producto),
      })
    }
  ...
  return {
    ...
    sitiosConflictoNombre: Array.from(sitiosConflictoNombreMap.values()),
    sitiosActualizarWms: Array.from(sitiosActualizarWmsMap.values()),
    resumenPorLocalidad,
    resumenPorModalidad,
  }
}
