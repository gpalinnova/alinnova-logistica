...
const SITE_COLS = [
  ...
  { key: 'direccion', label: 'DIRECCIÓN DE ENTREGA', w: 48 },
]
...
// Solo las columnas de producto cuyo menú del día tiene un producto asignado
// (id de producto resuelto, no null). Si el menú no trae ese producto, la
// columna se omite por completo y no ocupa espacio en el PDF.
function getActiveComponentes(menuDelDia) {
  return COMPONENTES.filter(c => menuDelDia?.[c.key])
}

function drawTableHeader(doc, y, menuDelDia, activeComponentes) {
  const rowH = 5.6
  ...
  const menuStartX = x
  const menuW = activeComponentes.length * COMPONENTE_COL_W * 2
  drawCell(doc, menuStartX, y, menuW, rowH, 'MENU', { bold: true, fontSize: 6 })

  for (const comp of activeComponentes) {
...
function drawSiteRow(doc, y, fila, rowH, activeComponentes) {
  let x = MARGIN
  const siteValues = {
...
function drawTotalRow(doc, y, totales, activeComponentes) {
  const rowH = 6.5
  ...
  for (const comp of activeComponentes) {
    drawCell(doc, x, y, COMPONENTE_COL_W, rowH, fmtN(totales.canastillasRecalc[comp.key]), { bold: true, fontSize: 6.6, fill })
    x += COMPONENTE_COL_W
    drawCell(doc, x, y, COMPONENTE_COL_W, rowH, fmtN(totales.unidadesSueltasRecalc[comp.key]), { bold: true, fontSize: 6.6, fill })
    x += COMPONENTE_COL_W
  }
...
function drawProductosLoteBlock(doc, y, w, menuDelDia, activeComponentes) {
  const rowH = 5.2
  const headerH = 5
  const cols = [
    { label: 'PRODUCTO', w: w * 0.35 },
    { label: 'LOTE', w: w * 0.40 },
    { label: 'F. VENCIMIENTO', w: w * 0.25 },
  ]
  let x = MARGIN
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(6.4)
  ...
  let ry = y + headerH
  const numFilas = Math.max(5, activeComponentes.length)
  for (let i = 0; i < numFilas; i++) {
    x = MARGIN
    const producto = i < activeComponentes.length ? menuDelDia[activeComponentes[i].key]?.nombre || '' : ''
    cols.forEach(c => {
      const texto = c.label === 'PRODUCTO' ? producto : ''
      drawCell(doc, x, ry, c.w, rowH, texto, { fontSize: 5.6 })
      x += c.w
    })
    ry += rowH
  }
...
export function generateRuteroPDF(conductores, { fechaISO, menuDelDia }) {
  ...
  const activeComponentes = getActiveComponentes(menuDelDia)
  ...
  conductores.forEach((c, i) => {
    ...
    y = drawTableHeader(doc, y, menuDelDia, activeComponentes)
    ...
    for (const fila of c.filas) {
      ...
      y = drawSiteRow(doc, y, fila, rowH, activeComponentes)
    }

    y = drawTotalRow(doc, y, c.totales, activeComponentes)
    ...
    const productosBottom = drawProductosLoteBlock(doc, y, leftW, menuDelDia, activeComponentes)
    let ry = y
