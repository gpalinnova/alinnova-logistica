...
function drawTableHeader(doc, y, menuDelDia) {
  ...
  // MENU: fila 1 = "MENU" abarcando todo; fila 2 = componente; fila 3 = producto + empaque; fila 4 = CAN/UND
  const menuStartX = x
  ...
  for (const comp of COMPONENTES) {
    ...
    drawCell(doc, x, y + rowH * 3, w2, rowH, empaqueTexto(producto), { fontSize: 4.8 })
    x += w2
  }

  // Superponer separación CAN/UND en la última fila
  x = menuStartX
  for (let i = 0; i < COMPONENTES.length; i++) {
    drawCell(doc, x, y + rowH * 3, COMPONENTE_COL_W, rowH, '', {})
    drawCell(doc, x + COMPONENTE_COL_W, y + rowH * 3, COMPONENTE_COL_W, rowH, '', {})
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(4.6)
    doc.text('CAN', x + COMPONENTE_COL_W / 2, y + rowH * 3.5 + 1, { align: 'center' })
    doc.text('UND', x + COMPONENTE_COL_W * 1.5, y + rowH * 3.5 + 1, { align: 'center' })
    x += COMPONENTE_COL_W * 2
  }
