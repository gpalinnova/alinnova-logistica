...
const TIPO_HEADER_TARGETS = ['tipo a', 'tipo b', 'tipo c', 'tipo d', 'tipo n']
...
function findSheetAndHeaderRow(workbook) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    for (let r = 0; r < maxScan; r++) {
      ...
      const hasIdSitio = normalizedCells.some(h => h.includes(target))
      if (!hasIdSitio) continue
      const hasTipo = normalizedCells.some(h => TIPO_HEADER_TARGETS.includes(h))
      if (!hasTipo) continue
