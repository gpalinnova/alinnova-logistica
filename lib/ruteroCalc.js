...
export function buildRuteroConductores(asignaciones, sitiosById, baseByIdSitio, menuDelDia) {
  ...
  for (const a of asignaciones) {
    ...
    g.filas.push({
      ...
      nombreDescripcion: sitio.nombre_institucion || `Sitio ${a.id_sitio_entrega} (sin datos maestro)`,
  ...
  const conductores = Array.from(grupos.values()).map(g => {
    ...
    const totales = {
      tipoA: 0, tipoB: 0, tipoC: 0, tipoD: 0, muestra1: 0, muestra2: 0, total: 0,
      canastillas: {}, unidadesComponente: {}, canastillasRecalc: {}, unidadesSueltasRecalc: {},
    }
    for (const key of COMPONENTE_KEYS) {
      totales.canastillas[key] = 0
      totales.unidadesComponente[key] = 0
    }
    ...
    filas.forEach(f => {
      ...
      for (const key of COMPONENTE_KEYS) {
        totales.canastillas[key] += f.componentes[key].canastillas
        totales.unidadesComponente[key] += f.cantidadesComponente[key]
      }
    })

    for (const key of COMPONENTE_KEYS) {
      const { canastillas, unidades } = calcularCanastillas(totales.unidadesComponente[key], menuDelDia?.[key]?.unidades_por_canastilla)
      totales.canastillasRecalc[key] = canastillas
      totales.unidadesSueltasRecalc[key] = unidades
    }

    totales.totalCanastas = COMPONENTE_KEYS.reduce((s, key) => {
      const llenas = totales.canastillasRecalc[key] || 0
      const sueltas = totales.unidadesSueltasRecalc[key] || 0
      return s + llenas + (sueltas > 0 ? 1 : 0)
    }, 0)
