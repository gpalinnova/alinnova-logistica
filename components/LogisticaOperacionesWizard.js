              ...
              {localidades.filter(l => l.selected && l.numRutas > 1).map(l => {
                ...
                return (
                  <div key={l.nombre} className="wizard-ruta-detail">
                    ...
                    {rutasLoc.length > 1 && (
                      <div className="subrutas-resumen">
                        {rutasLoc.map(r => {
                          const stats = getStatsRuta(r)
                          return (
                            <div key={r.id} className="subrutas-resumen-item">
                              <div className="wizard-ruta-nombre-static">{r.nombre}</div>
                              <div className="subrutas-resumen-kpis">{stats.ptos} PTOS · {fmtN(stats.cant)} UND · {stats.canast} CAN</div>
                            </div>
                          )
                        })}
                      </div>
